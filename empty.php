                    ...
                    <tr>
                        ...
                        <td><a class="btn btn-primary" href="bill.php?id=<?php echo $row['booking_id']; ?>">Bill</a></td> 
                         <?php } else if($row['paid']==1 ) { ?>
                          ...
                          <?php echo ($row['paid'] == '0') ? '<td>No</td>' : '<td>Yes</td>'; ?>
...
$query = "SELECT * FROM booking WHERE booking_id=" . (int)$id;
$result = pg_query($connection, $query);

if ($result && pg_num_rows($result) > 0) {
    $row = pg_fetch_assoc($result);
    // Check if each key is set before using it
    $department = isset($row['department']) ? $row['department'] : '';
    $type = isset($row['type']) ? $row['type'] : '';
    $req_date = isset($row['req_date']) ? $row['req_date'] : '';
    // Repeat this for other keys...
} else {
    echo "No results found or query failed: " . pg_last_error();
}
?>
